...
class IntegracionController extends BaseApiController
{
    ...
    public function sincronizarMatriculaEstudiante()
    {
        ...
        $cantMatriculas = 0;
        foreach ($ud_programadas as $id_sigi_programacion_ud => $id_moodle_course) {
    ...
    public function getSectionData()
    {
        ...
        if ($ies['MOODLE_SYNC_ACTIVE'] > 0) {
            ...
            $this->json([
                'success' => true,
                'message' => json_encode($this->serviceMoodle->getSectionData($id_section, $courseid, $MOODLE_URL, $MOODLE_TOKEN))
            ]);
            exit;
        } else {
            $this->json([
